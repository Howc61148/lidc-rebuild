...
上表沿用同一權重，但輸入範圍、參考標準、聚合與配對設定同時變更，不能把差異歸因於單一修正，也不是模型退步或改善的受控對照。初版程式保留為 `evaluate_v1_sampled.py`，**不再用於產生目前成果**；過程與更正註記見 [`docs/WORKLOG.md`](docs/WORKLOG.md)。

**比較上的說明**：本專案參考 LUNA16 的 FROC／CPM 指標形式與中心距離配對概念，但使用自行切分的 LIDC-IDRI 子集、自行建立的參考標準與本專案評分實作，**不是完整 LUNA16 官方流程，也不是官方榜單結果**。相同指標名稱或共識門檻，不代表不同實驗的分數可直接比較。方法定義見 [`docs/METRICS.md`](docs/METRICS.md)。
