...
最終模型為 YOLO11s、影像尺寸 960。於 104 位病人、189 顆結節的**測試集**（病灶層級）：

| 指標 | 數值 |
|---|---|
| 召回率 @ 2 FP/scan | **0.878**（95% CI: 0.833–0.920） |
| CPM（LUNA16 七點平均） | **0.844** |

FROC 曲線：

| FP/scan | 0.125 | 0.25 | 0.5 | 1 | 2 | 4 | 8 |
|---|---|---|---|---|---|---|---|
| 召回率 | 0.693 | 0.772 | 0.825 | 0.862 | 0.878 | 0.926 | 0.947 |

測試集於模型定案後僅評估一次。驗證集上的對應數值為 CPM 0.863、召回率 0.904，差距落在信賴區間重疊範圍內。
...
**評估層級** — 原評估以切片為單位。結節橫跨多張切片，此方式無法回答臨床關心的「整顆結節是否被發現」。以同一模型（YOLO11n，696 位病人）比較，切片層級召回率為 0.684，改以病灶層級評估後為 0.885——差異完全來自評估方式，模型並未改變。
...
- 在 8 FP/scan 的寬鬆條件下召回率仍為 0.947，代表約 5% 的結節在任何門檻下皆未被偵測。此為方法本身的上限，非門檻設定問題
